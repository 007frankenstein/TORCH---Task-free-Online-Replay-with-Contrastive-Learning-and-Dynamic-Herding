...
#         if "z_value" in self.data_frame.columns:
#             z_val = self.data_frame.iloc[idx]["z_value"]
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "z_value" in self.data_frame.columns:
            z_val = self.data_frame.iloc[idx]["z_value"]
        if "feature" in self.data_frame.columns:
            features = self.data_frame.iloc[idx]["feature"]
...
def get_train_collection_name(dataset, exp, rnd, n_cls, iter):
    collection_name = "{dataset}_train_{exp}_rand{rnd}_cls{n_cls}_task_{iter}".format(
...
def get_test_datalist(args, exp_name: str, cur_iter: int) -> List:
    ...
    if args.mode == "joint":
        ...
        if exp_name in ["blurry10", "blurry30"]:
    ...
    datalist = []
    for iter_ in tasks:
        collection_name = "{dataset}_test_{exp}_rand{rnd}_cls{n_cls}_task_{iter}".format(
            dataset=args.dataset, exp=args.exp_name, rnd=args.rnd_seed , n_cls=args.n_cls_a_task, iter=iter_
        )
        datalist += pd.read_json(
            f"collections/{args.dataset}/{collection_name}.json"
        ).to_dict(orient="records")
...
def get_statistics(dataset: str):
    ...
    mean = {
        ...
        "core50": (0.2650071382522583, 0.2650071382522583, 0.2650071382522583),
    }
    ...
    std = {
        ...
        "core50": (0.32422345876693726, 0.3247722387313843, 0.3252220153808594),
    }
